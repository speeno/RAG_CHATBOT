...
- **`/embed`** — 관리 메뉴 없이 상담 화면만 노출되는 경로(2026-08-20). 최종 사용자에게 이 URL만 공유하면 되고, iframe 임베드에도 사용 가능. 관리자 화면은 별도로 `ADMIN_TOKEN` 게이트로 보호됨.
...
| # | 화면 | 목업 근거 | 비고 |
|---|---|---|---|
| N1 | 상담 이력 | user nav | ✅ 2026-08-20 — `/history` (`HistoryView.tsx`): localStorage 대화 목록(50건), 상세 보기, '이어서 상담하기'(`/?conversation=`으로 복원), 삭제 |
| N2 | 도움말 | user nav | ✅ 2026-08-20 — `/help` 정적 페이지(소개·이용 방법·FAQ) |
| N3 | 설정 | admin nav | ✅ 2026-08-20 — `/admin/settings` 읽기 전용(`GET /api/admin/settings`: LLM/임베딩/검색/청킹/보안). 값 변경은 여전히 env |
| N4 | 카테고리/태그 관리 | admin/03 nav | ✅ 2026-08-20 — `/admin/taxonomy`: 카테고리 CRUD(이름 변경·삭제 시 문서 일괄 반영, 재할당), 태그 집계·이름 변경·제거. 문서에 `tags`(JSON)·업로드 폼 태그 입력 추가 |
| N5 | 권한 관리 | admin/03 nav | ✅ 2026-08-20 — `/admin/access`: 문서별 `access_level`(public/internal). **검색 단계 필터**(PRD §29): 익명 채팅=public만, 관리자 검색 테스트='내부 문서 포함' 토글. 역할별 RBAC은 후속 |
| N6 | 모니터링 | admin/03 nav | ✅ 2026-08-20 — `/admin/monitoring`: 색인 작업 표(진행률·처리 시간·오류·재색인), 시스템 상태(가동 시간·DB·미처리 문의), 5초 자동 갱신. `GET /api/admin/monitoring` |
...
| N9 | 문서 검색 | admin/02 검색바 | ✅ 2026-08-20 — 지식베이스 목록에 검색 입력(문서명/ID/카테고리/파일명, 클라이언트 필터). Topbar 통합 검색은 생략 |

### B. 화면은 구현됐으나 목업 요소가 빠진 것

| 화면 | 빠진 요소 (목업 기준) | 현재 구현 |
|---|---|---|
| 부정 피드백 (user/04) | ✅ 2026-08-20 — 인라인 패널로 사유 복수 선택 + 추가 의견(0/500) + '상담원에게 전달'(escalate→inquiries 접수) + 제출/취소. 전용 페이지 대신 인라인 유지(의도적) | `POST /api/feedback {reasons[],comment,escalate}` |
| 문서 업로드 (admin/03) | ⚠️ 부분 완료 2026-08-20 — **PDF 지원(pypdf)**·**여러 파일**·10MB·상태 배지에 **진행률 %** 추가. 남은 것: DOCX, 보안등급/태그 메타, 임시 저장, 별도 작업 이력 테이블 | |
