...
- `providers/weather.py` — data.go.kr VilageFcstInfoService_2.0 client (`KMA_SERVICE_KEY`, off when unset): 실황+오늘/내일 예보, region nx/ny map, 10-min cache; orchestrator injects a `KMA-LIVE` context chunk for weather-intent queries (`is_weather_query`/`detect_region`) and bypasses fail-closed only when live weather grounding exists. Region priority: explicit region name in query > browser geolocation (`ChatRequest.location {lat,lon}` → `latlon_to_grid` LCC conversion, frontend stores it in localStorage `rag.geo` after one prompt on first weather question) > default 서울. Debug: `GET /api/admin/weather`.
...
- `rag/retriever.py` — `VectorStore` interface; `NumpyVectorStore` caches active+indexed chunk vectors **and a BM25 index** (`rag/bm25.py`: pure-Python Okapi, Hangul bigram tokenizer) in memory (invalidate on any doc change). Hybrid search = dense top-30 + BM25 top-30 → weighted RRF (dense 0.7) for **candidate selection**, final order by cosine score (keeps fail-closed threshold semantics); `RETRIEVAL_MODE=dense` to disable. `Retriever.retrieve_multi` unions multi-query results via RRF. `rag/multiquery.py` (LLM query expansion, off for extractive), `rag/reranker.py` (`RERANKER=llm` optional listwise). Only `status='active'` docs are searchable. Golden-set eval: `eval/golden.jsonl` + `scripts/eval_retrieval.py` (Recall@5 KPI gate), offline regression test in `tests/test_api.py::test_golden_recall_at_5_offline`.
- `rag/orchestrator.py` — the flow: history → cheap query rewrite for short/anaphoric follow-ups → `search()` (optional multi-query → hybrid retrieve → optional rerank) → **fail-closed if top score < threshold (no LLM call)** → `[Document N]` context + system prompt (`[1]` citation markers) → LLM stream → post-process (`_is_no_answer`, keep only cited sources, renumber) → save messages + `turn_logs`. Events: `meta → sources → delta* → done`.
- `api/routes.py` — REST + SSE (`/api/chat/stream`); two routers: `router` (public: health/chat/conversations/feedback/inquiries POST) and `admin` (knowledge/logs/stats/search-test/inquiries GET·PATCH, guarded by `require_admin` when `ADMIN_TOKEN` is set). `core/stats.py` aggregates `turn_logs` in Python (KST days) for `/api/stats/*`. Tests in `tests/` use `TestClient` with env-forced offline providers (see `conftest.py`); `seeded` fixture indexes `sample_docs/` synchronously (`sync=true`).

## Frontend (`frontend/`)

App Router, no UI framework; `app/globals.css` carries the design tokens copied from `docs/design/html/shared.css`. `lib/api.ts` is the typed client + SSE parser (keep in sync with `backend/app/api/schemas.py`). Pages: `/` (`components/chat/ChatView.tsx` + `Messages.tsx`: streaming, citation badges, source cards, fail-closed handoff card, 👍/👎 with reasons) and `/admin/knowledge` (`components/admin/KnowledgeView.tsx`: upload with metadata, processing-status polling, activate/deactivate, reindex, delete, chunk preview), `/admin/search-test` (`components/admin/SearchTestView.tsx`: query → normalization/rewrite/threshold verdict/Top-k hit/result table/chunk preview via `POST /api/search/test`; BM25/Reranker columns are Phase 2 placeholders), `/admin/logs` (`components/admin/LogsView.tsx`: filtered/paged turn logs via `GET /api/logs`, detail panel, full-conversation view, CSV export), `/admin` (`DashboardView.tsx` + `charts.tsx` SVG charts over `GET /api/stats/overview`), `/admin/unanswered` (`UnansweredView.tsx`: `GET /api/stats/unanswered`, review status, inquiries). `app/admin/layout.tsx` wraps admin pages in `AdminGate` (token login when backend `ADMIN_TOKEN` is set; `lib/api.ts` `f()` attaches `Authorization: Bearer` and emits `rag:unauthorized` on 401). Fail-closed handoff card posts `POST /api/inquiries`. User pages: `/history` (localStorage conversation list, resume via `/?conversation=`), `/help` (static). Feedback panel sends `{reasons[],comment,escalate}`; escalate also files an inquiry. Uploads accept multiple files incl. PDF (pypdf, 10MB); knowledge list has client-side text search; `/admin/settings` renders `GET /api/admin/settings` read-only. `/admin/taxonomy` (categories CRUD w/ cascade rename+reassign, tags aggregate/rename/delete), `/admin/access` (per-doc `access_level` public/internal — filtered at retrieval: anonymous chat sees public only, search-test has include_internal toggle), `/admin/monitoring` (`GET /api/admin/monitoring`, 5s auto-refresh, reindex). `/embed` is the user-only chat route (no app shell — `components/Shell.tsx` drops Sidebar/Topbar for `/embed*`); share this URL with end users. `NEXT_PUBLIC_API_URL` defaults to `http://localhost:8000`.
