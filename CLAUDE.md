...
Config is `backend/.env` (copy from `.env.example`; **no inline `#` comments on value lines** — python-dotenv would read them as the value). Provider resolution is `auto`: LLM = Anthropic (`ANTHROPIC_API_KEY`) → OpenAI (`OPENAI_API_KEY` or `OPENAI_API_KEY_FILE`, default `../openaikey`, key token is regex-extracted) → `extractive` offline fallback; embeddings = Voyage → OpenAI → `hash` n-gram fallback. Never print or commit the key file (`.gitignore`d).

## Backend architecture (`backend/app`)

- `core/config.py` — Settings + `NO_ANSWER_MESSAGE` (the canonical fail-closed string) + per-provider score thresholds. `core/db.py` — SQLite schema (documents/chunks(+embedding BLOB)/conversations/messages/turn_logs). `core/services.py` — wires everything; `app.state.services`.
...
- `rag/retriever.py` — `VectorStore` interface; `SqliteNumpyVectorStore` caches active+indexed chunk vectors in memory (invalidate on any doc change). Only `status='active'` docs are searchable.
